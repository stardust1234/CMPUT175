# CMPUT 175 Winter 2013 Lab 4 Exercise 5
...
def main():
    ...
    mazeSquare_stack = stack.Stack()
    mazeSquare_stack_unsolvable = stack.Stack()
    currentSquare = maze.get_start_square()
    mazeSquare_stack.push(maze.get_start_square())
    mazeSquare_stack_unsolvable.push(maze.get_start_square())

    while True:
          for move in currentSquare.get_legal_moves():
              mazeSquare_stack.push(move)
              #print(move.get_location())
              mazeSquare_stack_unsolvable.push(move)
          currentSquare = mazeSquare_stack.pop()
          if mazeSquare_stack.size() == 0:
             print('This maze is unsovable.')
             while mazeSquare_stack_unsolvable.size() !=0:
                   print(mazeSquare_stack_unsolvable.pop().get_location())
             break
          if maze.is_finish_square(currentSquare):
             print('This maze is sovalble.')
             print('With the finish square:' + str(currentSquare.get_location()))
             break
main()